...
async fn handle_retry_action(
    ...
    let last_message = session_manager.swap_session_pending_message(chat_id.clone(), None);
...
async fn handle_show_raw_action(
    ...
    let history_message = session_manager.with_mut_session(chat_id.to_string(), |session| {
...
async fn actually_handle_chat_message(
    ...
    let mut msgs = session_manager.get_history_messages(&chat_id);
    ...
    let reply_result = match result {
        Ok(res) => {
            let reply_history_message = session_manager.with_mut_session(chat_id.clone(), |session| {
            ...
            let need_fallback = if config.renders_markdown {
                ...
                if let Err(first_trial_err) = edit_message_text.await {
                    // TODO: test if the log::error is related to Markdown before
            ...
            session_manager.with_mut_session(chat_id.clone(), |session| {
        ...
        Err(err) => {
            ...
            session_manager.swap_session_pending_message(chat_id.clone(), Some(user_msg));
            ...
            bot.edit_message_text(chat_id, sent_progress_msg.id, &config.i18n.api_log::error_prompt)
...
async fn reset_session(
    ...
    session_manager.reset_session(chat_id.to_string());
